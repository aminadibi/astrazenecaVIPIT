...
a <- res2 %>% filter(R==1 & ve==0.60 & type == "A: 80+, 70-79, 60-69, FLW, 50-59, ...")
b <- res2 %>% filter(R==1 & ve==0.60 & type == "B: Oldest to Youngest") 
```
...
In our analysis, Scenario A led to `r round(b$cases - a$cases, 0)` fewer cases of COVID-19, `r round(b$hosp - a$hosp, 0)` fewer hospitalizations, `r round(b$deaths - a$deaths, 0)` fewer deaths, and `r round(b$long - a$long, 0)` fewer cases of Long COVID, assuming $R_0=1$. Appendix B includes results of the sensitivity analysis for a wider range of values for $R_0$ and the effectiveness of the vaccine against transmission, $v_e$.
